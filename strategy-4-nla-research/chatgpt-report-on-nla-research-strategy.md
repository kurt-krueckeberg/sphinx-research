...
flowchart TD
    ...
    C --> D[Search S 8 church books by parish + date span]
    D --> E{Family found?}
    E -- Yes --> F[Extract household cluster<br/>parents, siblings, sponsors, farm/house]
    E -- No --> G[Broaden to neighboring parish and variant surname spellings]
    G --> D
    F --> H[Search court and Amt records<br/>H 120 / L 101 + village + surname]
    H --> I[Search central-government files<br/>L 2 / L 3 / L 4 + emigration terms]
    I --> J[Check emigration-specific classifications and indexes]
    J --> K[Triaging each hit<br/>Title, Life span, Index Person/Place, Includes]
    K --> L{Digital copy available?}
    L -- Yes --> M[Open viewer, read key lines first]
    L -- No --> N[Record full signature and order/consult in reading room]
    M --> O[Correlate findings across parish, court, and administrative records]
    N --> O
    O --> P[Build proof set for identity, family link, and departure]
```
